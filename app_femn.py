...
=================================================
Includes strict hover isolation, Market Comparison, Steel VIU (70% for MC),
...
with st.sidebar:
    ...
    # UPDATED: MC FeMn now defined as 70%
...
c6.metric("Scaling Factor", f"{meta.get('scaling_factor', 0):.4f}", help="Based on 1-Year Rolling Average")
...
with tab3:
    ...
    if available_drivers:
        ...
        # We normalize them to the first day of the currently displayed historical window
...
# ══ TAB 4: STEEL CALCULATOR WITH CARBON PENALTY ══════════════════════════════
with tab4:
    ...
    # UPDATED: MC FeMn defaults to 70.0%
    ...
    fig3.update_layout(**_layout(f"Raw Price vs. True Metallurgical Cost (Mn={mn_pct}%, C={c_pct}%)", "Raw Price (Rs/Kg)", 450))
